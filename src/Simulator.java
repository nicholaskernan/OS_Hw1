...
public class Simulator {
	public static void main(String[] args) {
		...
		System.out.println("In this version, a low priority process can finish its timeslice when a higher priority process arrives, but after that it will be kicked off the processor. ");
		System.out.println();
		...
		for(int i = 0; i < finishedProcesses.size(); i++) {
			...
			if(i < 10) {
				System.out.println(i + "        " + process.priority + "        " + process.init_burstTime + "        " + String.format("%.2f %s %.2f %s %.2f            %.2f", process.arrivalTime, spaces1, process.completionTime, spaces2, process.responseTime, process.turnaroundTime));
			} else {
				System.out.println(i + "       " + process.priority + "        " + process.init_burstTime + "        " + String.format("%.2f %s %.2f %s %.2f            %.2f", process.arrivalTime, spaces1, process.completionTime, spaces2, process.responseTime, process.turnaroundTime));
			}
		}
